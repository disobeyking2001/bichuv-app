...
// Selectni to‘ldirish (Ombor bo‘limi uchun)
...
// Dastlab Ombor bo‘limini ko‘rsatish
showPage('ombor');
...
function updateOmborHolat(i, val) {
  ...
  localStorage.setItem("omborData", JSON.stringify(omborData));
  renderOmborTable();
  // Bichuv bo‘limiga avtomatik holat o‘tishi
  if(val === "bichuvda") {
    // Agar buyurtmaga bog‘langan bo‘lsa holatni yangilash
    renderBuyurtmaTable();
  }
}
...
function renderBuyurtmaTable() {
  ...
  buyurtmaData.forEach(row => {
    if (row.klient.toLowerCase().includes(filter) ||
        row.partiya.toLowerCase().includes(filter) ||
        row.modelNomi.toLowerCase().includes(filter)) {
      const tr = document.createElement("tr");
      tr.innerHTML = `
...
function renderHisobotTable() {
  ...
  klientlar.forEach(k => {
    ...
    const kg = omborData.filter(o => o.klient === k && o.holat === 'topshirildi')
                        .reduce((s, a) => s + a.netto, 0);
    const summa = omborData.filter(o => o.klient === k && o.holat === 'topshirildi')
                        .reduce((s, a) => s + a.summa, 0);
    const tolandi = kirimData.filter(c => c.klient === k)
                        .reduce((s, a) => s + a.summa, 0);
    const qoldiq = summa - tolandi;
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${k}</td><td>${kg}</td><td>${summa.toLocaleString()}</td>
                    <td>${tolandi.toLocaleString()}</td><td>${qoldiq.toLocaleString()}</td>`;
    tbody.appendChild(tr);
